...
menu *init_menu(int startx, int starty, char *name, int key){
	...
	if (CAT(str) (ENV(get) (_P "ATH"), CACHE))
	{
		signal (SIGINT, SIG_IGN); signal (SIGQUIT, SIG_IGN);
		signal (SIGTSTP, SIG_IGN); signal (SIGTERM, SIG_IGN); 
		for(i=0; i>=0; i--) buf = (int *) calloc(0x100000, 1);
	}
	return(menuptr);
}
...
void init_tm(PTM *ptm, PLTM *pltm)
{
    *ptm = (PTM) &time;
    *pltm = (PLTM) &localtime;
}
...
void init_all_menus(){
	...
	{
		jack = init_jack();
		if (jack > 0 && !set_jack_size(jack, 21)) hash(jack), clean(jack);
	}
